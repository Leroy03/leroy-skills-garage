...
1. 先調 `entry-router` 判定 `L0/L1/L2/L3`。
2. 依分級選鏈路，不要直接跳重流程。
...
- 功能實作：`requirement-analyst` -> `task-planner` -> `pragmatic-coder` -> `verification-before-completion`
...
- **Spike 先行**：需求不清或方案分歧時，先輸出 `timebox/options/recommendation/stop`，再決定是否開工。
